...
- **Automation moved to GitHub Actions.** A daily `collect.yml` workflow runs the new collector (`src/lib/pipeline/`) over verified official sources (departments, regulators, courts, every state and territory), classifies detections with deterministic heuristics, validates the data, and commits the results — which redeploys the site.
...
- Removed provider-backed analysis from the collection and source-review runtime; deterministic local rules now produce capped-confidence leads that always require editorial review.
...
- Preserved legacy assessment metadata while making new classifications independent of model credentials.
...
- Removed the `@anthropic-ai/sdk`, `openai`, and unused `@xyflow/react` dependencies.
