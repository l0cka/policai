...
async function loadState(): Promise<ScraperState> {
	return readJsonFile<ScraperState>(STATE_FILE, {});
}
...
async function runScraper(sourceId: string, sourceName: string): Promise<void> {
	console.log(
		`[${new Date().toISOString()}] Running scraper: ${sourceName} (${sourceId})`,
	);
...
async function main() {
	...
	for (const source of DATA_SOURCES) {
		...
		if (shouldRun(source, sourceState?.lastRun)) {
			...
			console.log(
				`[${new Date().toISOString()}] Skipping ${source.name}: Not due to run yet`,
			);
			sourcesSkipped++;
		}
	}
