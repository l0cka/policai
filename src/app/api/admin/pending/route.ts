...
export async function GET(request: Request) {
	...
	try {
		...
		if (status && !isSourceReviewStatus(status)) {
		...
		const filters =
			status && isSourceReviewStatus(status) ? { status } : undefined;
		const reviews = await getSourceReviews(filters);
...
export async function POST(request: Request) {
	...
	try {
		...
		if (!isSourceReviewEntryKind(rawEntryKind)) {
			return NextResponse.json(
				{ error: "Invalid entryKind", success: false },
				{ status: 400 },
			);
		}

		const entryKind = rawEntryKind;
		const suggestedJurisdiction = normalizeJurisdiction(
			analysis.jurisdiction || analysis.suggestedJurisdiction,
		);
		const suggestedType = normalizePolicyType(
			analysis.policyType || analysis.suggestedType,
		);
		const now = new Date().toISOString();
		const reviewTitle = title || "Untitled";
		const policyId = buildPolicyId(reviewTitle);
		const proposedRecord: Policy | TimelineEvent =
		...
		const review = await createSourceReview({
			...
			discoveredAt: now,
			createdBy:
				typeof user.email === "string" ? user.email : user.id || "admin",
			notes,
			analysis: {
				isRelevant: analysis.isRelevant,
				relevanceScore: analysis.relevanceScore,
				suggestedType:
					analysis.policyType || analysis.suggestedType ? suggestedType : null,
		...
		return NextResponse.json(
			{
				error:
					error instanceof Error && error.name === "DuplicatePolicyError"
