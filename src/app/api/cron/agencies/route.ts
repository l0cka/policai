...
export async function GET(request: Request) {
	...
	console.log(
		`[cron/agencies] Starting agency discovery at ${new Date().toISOString()}`,
	);

	try {
		// Run discovery. Insert/update decisions are handled below against Supabase.
		const updates = await runAgencyDiscoveryAgent();
			...
			for (const update of updates) {
				...
				if (existing) {
					...
					if (!error) updated++;
						...
						console.warn(
							`[cron/agencies] Failed to update ${update.name}:`,
							error.message,
						);
				} else {
					// Insert new agency
					const { error } = await supabase.from("agencies").insert(update);

					if (!error) created++;
					else
						console.warn(
							`[cron/agencies] Failed to insert ${update.name}:`,
							error.message,
						);
				}
			}
		...
		console.log(
			`[cron/agencies] Complete. Discovered: ${updates.length}, Created: ${created}, Updated: ${updated}`,
		);
		...
		return NextResponse.json(
			{
				success: false,
				error:
					error instanceof Error ? error.message : "Agency discovery failed",
			},
			{ status: 500 },
		);
	}
