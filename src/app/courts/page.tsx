	...
	useEffect(() => {
		fetch("/api/policies")
			...
			.catch((err) => {
				...
				setError(
					"Unable to load court practice notes. Please try refreshing the page.",
				);
			})
			...
			{jurisdictionsWithout.length > 0 && (
				<section className="mt-10">
					...
					<p className="text-xs text-muted-foreground/60 mt-3">
						These jurisdictions have not yet issued public AI practice notes for
						their courts. This tracker updates as new instruments are published.
					</p>
				</section>
			)}
		</div>
