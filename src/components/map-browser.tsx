...
export function MapBrowser({ policiesData }: { policiesData: Policy[] }) {
	...
	const selectedPolicies = useMemo(() => {
		...
		return policiesData.filter(
			(p) => p.jurisdiction === selectedJurisdiction && p.status !== "trashed",
		);
		...
		<div className="relative flex min-h-[calc(100svh-7rem)] overflow-hidden bg-background">
			...
			>
				<div
					...
					aria-live="polite"
				>
					{policiesData.filter((p) => p.status !== "trashed").length} policies
					across {Object.keys(jurisdictionData).length} jurisdictions
					{selectedJurisdiction && (
						<span>
							{" "}
							&middot; Viewing {JURISDICTION_NAMES[selectedJurisdiction]}
						</span>
					)}
				</div>
			</div>
