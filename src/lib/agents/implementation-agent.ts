...
Respond in JSON format:
{
  ...
  "tags": ["relevant tags"]
	...
	const jsonResult = extractJsonFromResponse<Omit<
		Policy,
	...
	if (jsonResult) {
		return {
			...jsonResult,
			jurisdiction: normalizeJurisdiction(jsonResult.jurisdiction),
			type: normalizePolicyType(jsonResult.type),
			status: normalizePolicyStatus(jsonResult.status),
			agencies: Array.isArray(jsonResult.agencies) ? jsonResult.agencies : [],
	...
	return {
		...
		jurisdiction: normalizeJurisdiction(finding.suggestedJurisdiction),
		type: normalizePolicyType(finding.suggestedType),
		status: "active",
...
export async function runImplementationAgent(
	...
	for (const finding of verifiedFindings) {
		try {
			...
			const normFindingUrl = finding.sourceUrl
				? normalizeUrl(finding.sourceUrl)
				: "";
			const existingPolicy = policies.find((p) => {
				...
				if (
					normFindingUrl &&
					p.sourceUrl &&
					normalizeUrl(p.sourceUrl) === normFindingUrl
				)
					return true;
				return false;
			});
			...
			if (existingPolicy) {
				...
				await updatePolicyInDb(existingPolicy.id, {
					...
					agencies: [
						...new Set([...existingPolicy.agencies, ...policyData.agencies]),
					],
				});
	...
	console.log(
		`[Implementation Agent] Complete. Created: ${createdCount}, Updated: ${updatedCount}, Skipped: ${skippedCount}`,
	);
