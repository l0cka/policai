...
import { normalizeJurisdiction, type Jurisdiction } from "@/types";
 ...
 */
export async function runAgencyDiscoveryAgent(): Promise<AgencyUpdate[]> {
	...
	console.log(
		`[Agency Discovery] Starting search with ${SEARCH_QUERIES.length} queries`,
	);
...
List each page with its URL and the agency name. Focus on pages where a specific government agency describes how it uses AI, its AI transparency commitments, or its AI usage policies.`,
			...
			const citationUrls = citations.filter((c: string) =>
				c.includes(".gov.au"),
			);
	...
	const urlsToProcess = Array.from(discoveredUrls.entries()).slice(
		0,
		MAX_DISCOVERED_URLS,
	);
	console.log(
		`[Agency Discovery] Found ${urlsToProcess.length} URLs to process`,
	);
...
If the page does not contain an AI transparency statement from a specific agency, return: {"hasStatement": false}`,
			...
			if (parsed.hasStatement && parsed.agencyName) {
				...
				agencyUpdates.push({
					...
					level: jurisdiction === "federal" ? "federal" : "state",
					jurisdiction,
	...
	console.log(
		`[Agency Discovery] Complete. Found ${results.length} agency transparency updates`,
	);
