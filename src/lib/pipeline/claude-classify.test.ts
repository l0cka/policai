...
vi.mock('./classifier-cli', () => ({
  runClassifier: vi.fn(),
}));

import { CLAUDE_BATCH_SIZE, classifyBatch } from './claude-classify';
import * as classifierCliModule from './classifier-cli';

const runClassifier = vi.mocked(classifierCliModule.runClassifier);
...
describe('classifyBatch', () => {
  it('returns one validated verdict per candidate', async () => {
    runClassifier.mockResolvedValueOnce(JSON.stringify([
  ...
  it('tolerates prose around the JSON array', async () => {
    runClassifier.mockResolvedValueOnce(
